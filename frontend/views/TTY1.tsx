      ...
      <div className="flex-1 flex flex-col bg-black">
        {activeSession && (
          <>
            ...
            {/* Status Bar */}
            <div className="h-6 bg-hlx-panel border-t border-hlx-border/40 flex items-center justify-between px-4 text-[10px] font-mono text-hlx-muted">
              <span>/DEV/TTY1</span>
              <button
                onClick={() => setLinkMode(m => m === 'SIMULATION' ? 'BRIDGE' : 'SIMULATION')}
                className="hover:text-hlx-text transition-colors"
              >
                LINK: {linkMode}
              </button>
            </div>
